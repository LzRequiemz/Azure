...
-- Bottom-right toasts — pulled up from screen edge so they sit mid–lower corner
notificationHolder.Position = UDim2.new(1, -24, 1, -96)
notificationHolder.Size = UDim2.new(0, 420, 0.48, -48)
...
function window:Notify(Info)
	...
	local legacyNotifyFont = Enum.Font.Code
	local notifyFontFace = nil
	do
	...
	local useRich = typeof(message) == "string" and string.find(message, "<", 1, true) ~= nil

	local function textBoundsForWrap(innerWrapWidth)
		local w = math.max(1, math.floor(innerWrapWidth))

		local function boundsFallback()
		...
		if notifyFontFace ~= nil then
			local ok, bounds = pcall(function()
				local params = Instance.new("GetTextBoundsParams")
				params.Text = message
				params.RichText = useRich
				params.Font = notifyFontFace
				params.Size = textSize
				params.Width = w
				return TextService:GetTextBoundsAsync(params)
			end)
			if ok and bounds ~= nil then
				return bounds
			end
		end

		local okFB, fb = pcall(boundsFallback)
		if okFB and fb ~= nil then
			return fb
		end

		return Vector2.new(8, math.max(textSize + 6, math.ceil(textSize * 1.25)))
	end

	local loose = textBoundsForWrap(100000)
	if loose.X < 1 or loose.Y < 1 then
		loose = Vector2.new(math.max(loose.X, 8), math.max(loose.Y, math.ceil(textSize * 1.2)))
	end
